...
import {deleteOnlineStatus, deleteSocket, setOnlineStatus, setSocket} from "./redisSocketService.ts";

// The socket server runs withing this function
export function startSocketServer(ioServer: Server) {
    ...
    ioServer.on('connection', socket => {
        ...
        setSocket(userId, socket.id).then(() => console.log('Socket set for user:', userId))
        ...
        socket.on('disconnect', () => {
            // Remove the user's online status from Redis
            deleteOnlineStatus(userId).then(() => console.log('User online status set to false'))

            // Remove the socket id from Redis
            deleteSocket(userId).then(() => console.log('Redis socket key deleted for user:', userId))
        })
    })
}
